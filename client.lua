...
local Cfg = {
    Clothes = {
        Male = {
            ...
            ['arms'] = 42, --RAMIONA
            ['pants_1'] = 4, ['pants_2'] = 0, --SPODNIE
            ['shoes_1'] = 145, ['shoes_2'] = 2, --BUTY
        ...
        Female = {
            ['tshirt_1'] = 15, ['tshirt_2'] = 0,--TSHIRT
            ['torso_1'] = 78, ['torso_2'] = 5, --TUŁÓW
            ['arms'] = 34, --RAMIONA
            ['pants_1'] = 160, ['pants_2'] = 0, --SPODNIE
            ['shoes_1'] = 113, ['shoes_2'] = 1, --BUTY
            ['helmet_1'] = -1, ['helmet_2'] = 0, --CZAPKA_TUTAJ_-1_OZNACZA_BRAK_CZAPKI
            ['chain_1'] = 0, ['chain_2'] = 0, --ŁAŃCUCH
            ['mask_1'] = 0, ['mask_2'] = 0, --MASKA
...
function WyciagnijPojazd()
    if ESX.Game.IsSpawnPointClear(Config.Strefy.Spawn.Pos, 7) then
        ...
        elseif posiadaVana == false then
            ESX.Game.SpawnVehicle('boxville2', Config.Strefy.Spawn.Pos, Config.Strefy.Spawn.Heading, function(vehicle)
...
CreateThread(function()
    ...
    while true do
        Citizen.Wait(2)
        if PlayerData.job ~= nil and PlayerData.job.name == 'courier' or PlayerData.hiddenjob ~= nil and PlayerData.hiddenjob.name == 'courier' then
			local sleep = true
...
function LosujCel()
    local LosowyPunkt = math.random(1, 20)
...
CreateThread(function()
    ...
    while true do
		...
		if xxx ~= nil and yyy ~= nil and zzz ~= nil then
			...
			if Dystans <= 25.0 and (PlayerData.job ~= nil and PlayerData.job.name == 'courier' or PlayerData.hiddenjob ~= nil and PlayerData.hiddenjob.name == 'courier') and (not MaPaczke) then
				sleep = false
				ESX.ShowFloatingHelpNotification('~b~ ZABIERZ PACZKE Z VANA', vec3(xxx, yyy, zzz))
				local pojazd = GetClosestVehicle(Pozycja, 6.0, 0, 70)
				if IsVehicleModel(pojazd, `boxville2`) then
			...
			if Dystans <= 25 and MaPaczke and (PlayerData.job ~= nil and PlayerData.job.name == 'courier' or PlayerData.hiddenjob ~= nil and PlayerData.hiddenjob.name == 'courier') then
				sleep = false
...
function KolejnaDostawa()
    TriggerServerEvent('kurier:kolejnadostawa')
    Citizen.Wait(300)
    LosujCel()
end
...
function KoniecPracy()
    ...
    if IsPedInAnyVehicle(Gracz, false) then
        ...
        if IsVehicleModel(Van, `boxville2`) then
...
AddEventHandler('adrenalina_courier:enteredMarker', function(zone)
...
AddEventHandler('adrenalina_courier:exitedMarker', function(zone)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'courier' or PlayerData.hiddenjob ~= nil and PlayerData.hiddenjob.name == 'courier' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('adrenalina_courier:enteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('adrenalina_courier:exitedMarker', LastZone)
			end
...
CreateThread(function()
	while true do
        ...
        if CurrentAction and not IsDead then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'courier' or PlayerData.hiddenjob and PlayerData.hiddenjob.name == 'courier' then
...
function OpenBossMenu()
	local elements = {
		...
		{label = "Akcje szefa job2", value = '2'},
    }
    if PlayerData.job.name == 'courier' and PlayerData.job.grade >= 6 or PlayerData.hiddenjob.name == 'courier' and PlayerData.hiddenjob.grade >= 6 then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'courier_actionss', {
		...
		if data.current.value == '1' then
			if PlayerData.job.name == 'courier' and PlayerData.job.grade >= 7 or PlayerData.hiddenjob.name == 'courier' and PlayerData.hiddenjob.grade == 7 then
			...
			elseif PlayerData.job.name == 'courier' and PlayerData.job.grade == 6 or PlayerData.hiddenjob.name == 'courier' and PlayerData.hiddenjob.grade == 6 then
				TriggerEvent('esx_society:openBossMenu', 'courier', function(data, menu)
					menu.close()
		...
		elseif data.current.value == '2' then
			if PlayerData.job.name == 'courier' and PlayerData.job.grade >= 7 or PlayerData.hiddenjob.name == 'courier' and PlayerData.hiddenjob.grade == 7 then
				TriggerEvent('esx_society:openHiddenBossMenu', 'courier', PlayerData.hiddenjob.grade, function(data, menu)
					menu.close()
				end, { showmoney = false, withdraw = false, deposit = true, wash = false, employees = true})
			elseif PlayerData.job.name == 'courier' and PlayerData.job.grade == 6 or PlayerData.hiddenjob.name == 'courier' and PlayerData.hiddenjob.grade == 6 then
				TriggerEvent('esx_society:openHiddenBossMenu', 'courier', PlayerData.hiddenjob.grade, function(data, menu)
        ...
        elseif data.current.value == '3' then
			menu.close()
			exports['adrenalina_legaljobs']:OpenLicensesMenu('courier')
		end
